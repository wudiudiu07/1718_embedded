...
static ssize_t device_read(struct file *filp, char *buffer, size_t length, loff_t *offset) 
{
    ...
    ADXL345_REG_READ(I2C_virtual, ADXL345_REG_INT_SOURCE, &soc_interrupt); //read the source of interrupt
    ADXL345_XYZ_Read(I2C_virtual, XYZ);

    //ADXL345_REG_READ(I2C_virtual, ADXL345_REG_INT_ENABLE, (uint8_t *)&interrupt_reg);

    // Copy time
    if (0 >= sprintf(chardev_msg, "%2x %4d %4d %4d %2d\n", soc_interrupt, XYZ[0], XYZ[1], XYZ[2], mg_per_lsb)) {
        printk (KERN_ERR "%s:%d\n", __FILE__, __LINE__);
    }
    printk (KERN_ERR "the soc_interrupt is %d\n", soc_interrupt);
...
static int __init init_driver(void) 
{
    ...
    resolution=XL345_10BIT; range=XL345_RANGE_16G; rate=XL345_RATE_12_5; // Set default values
    mg_per_lsb = ROUNDED_DIVISION(16*1000, 512); // 10-bit 2's complement -> gives 9 bits for range
    ADXL345_Init(I2C_virtual, &resolution, &range, &rate);

    //Initialize Tap Detection
    ADXL345_SETUP_TAP_DOUBLETAP(I2C_virtual);
