...
int main(int argc, char *argv[])
{
    ...
    if ((audio_FD = open ("/dev/IntelFPGAUP/audio", O_RDWR)) == -1){
        printf ("Could not open audio driver\n");
        return -1;
    }

    sprintf(command, "--");
    write (audio_FD, command, 2);

    // Setup audio - clears FIFO's
    sprintf(command, "init\n");
    write (audio_FD, command, strLenBytes(command));

    sprintf(command, "rate 8000\n");
    write (audio_FD, command, strLenBytes(command));
...
void write_to_audio_port(int value) {
    ...
    // Wait for space avaliable in write FIFO
    sprintf(command, "waitw\n");
    write (audio_FD, command, strLenBytes(command));

    // Write Data
    sprintf(command, "left %d\n", value);
    write (audio_FD, command, strLenBytes(command));
    sprintf(command, "right %d\n", value);
    write (audio_FD, command, strLenBytes(command));
}
