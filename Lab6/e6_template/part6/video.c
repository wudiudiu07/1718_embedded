...
static volatile unsigned int * char_ctrl_ptr;  // virtual address of character buffer controller
static volatile char *char_buffer; // used for virtual address of character buffer
static int text_resolution_x, text_resolution_y; // VGA screen size character buffer
...
static void get_screen_specs_text(void) 
{
    unsigned int resolutionReg = *(char_ctrl_ptr + 2);

    text_resolution_x = resolutionReg & 0xFFFF;
    text_resolution_y = resolutionReg >> 16;

    printk(KERN_ERR "Text ResolutionX: %d, ResolutionY: %d\n", text_resolution_x, text_resolution_y);
}
...
void write_text(int x, int y, char *string)
{
    ...
    while (string[0] != '\0') {
        buf = char_buffer + ((y << 7) | x); // base + y + x
        *buf = string[0];

        // Update
        ++string;
        ++x;
        if (x >= text_resolution_x) {
            x = 0;
            ++y;
            if (y >= text_resolution_y) {
                y = 0;
            }
        }
    }
    //printk(KERN_ERR "str:%sEnd\n", string);
}
...
{
    int col=0, row=0;
    for (row = 0; row < text_resolution_y; ++row) {
        for (col = 0; col < text_resolution_x; ++col) {
            write_text(col, row, " ");
        }
    }
}
...
static ssize_t device_write(struct file *filp, const char *buffer, size_t length, loff_t *offset) 
{
    ...
    // Character buffer
    else if (0 == strncmp("erase", cmd_string, strlen("erase"))) {
        clear_character();
    }
    else if (0 == strncmp("text", cmd_string, strlen("text"))) {
        int space_count = 0;
        int i = 0;
        sscanf(cmd_string, "text %d,%d", &x, &y);
        while (space_count != 2) {
            if (cmd_string[i] == ' ') {
                ++space_count;
            }
...
static int __init start_video(void) 
{
    ...
    char_buffer = (char *) ioremap_nocache (FPGA_CHAR_BASE, FPGA_CHAR_SPAN);
    if (char_buffer == 0) {
        printk (KERN_ERR "Error: ioremap_nocache returned NULL for character memory\n");
        goto error;
    }

    get_screen_specs_text();
