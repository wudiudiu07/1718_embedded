...
{   
    ...
    // short int
    short *buf = (short *)(pixel_buffer + ((y << 10) | (x << 1))); // base + y + x
    *buf = color; // & 0xFFFF;
}
...
void wait_for_vsync(void) {
    register int status;
    //printk(KERN_ERR "Before Wait:\n");

    *pixel_ctrl_ptr = 1; // start syncronication process
...
static ssize_t device_write(struct file *filp, const char *buffer, size_t length, loff_t *offset) 
{
    ...
    if (0 == strncmp("clear", cmd_string, strlen("clear"))) {
        ...
        *pixel_ctrl_ptr = 1; // Write 1 into buffer register
