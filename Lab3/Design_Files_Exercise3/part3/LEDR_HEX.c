...
static ssize_t device_write_ledr(struct file *filp, const char *buffer, size_t length, loff_t *offset)
{
    ...
    //printk(KERN_ERR"The buffer is %s \n",buffer);

    //chardev_msg[bytes] = '\0';
    //convert char to int
    //ledr_value = atoi(chardev_msg);
    //while(*chardev_msg){
    //    num=((*chardev_msg)-'0')+num*10;
    //    chardev_msg++;
    //    }
    //long ledr_value = strtol(chardev_msg, NULL, 16);
    //sscanf(chardev_msg, "%d", &ledr_value); //char array to int
    
    //printk(KERN_ERR"The chardev_msg is %s \n",chardev_msg);
    for (i = 2; i >=0 ; i--){
        integer = hex2int(chardev_msg[i]);
        if (integer != -1){
            num +=  integer * base;
            //printk(KERN_ERR"The integer is %d \n",integer);
            base = base * 16;
        }
    }
    
    //printk(KERN_ERR"The LEDR value is %d \n",num);
    
    *LEDR_ptr = (unsigned int) num;
...
static ssize_t device_write_hex(struct file *filp, const char *buffer, size_t length, loff_t *offset)
{
    ...
    //convert char to int
    //hex_value = atoi(chardev_msg);
    //printk(KERN_ERR"%s",chardev_msg);
    //sscanf(chardev_msg, "%d", &hex_value); 
